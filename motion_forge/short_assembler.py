...
def modo_masivo(target_id: str = None):
    ...
    for folder in carpetas:
        ...
        if not clips:
            print(f"⚠️ [Assembler] Saltando {fid}: No tiene videos .mp4 extractados de Meta AI aún en forja_local.")
            continue
        
        # Buscar en forja_local, imagenes_shorts, o en el directorio oficial sesion_ID
        wav = None
        posibles_rutas = [
            folder, 
            Path("output/imagenes_shorts") / fid,
            Path(f"output/sesion_{fid}"),
            Path("output/imagenes_shorts")
        ]
        
        for ruta_base in posibles_rutas:
            if not ruta_base.exists() or not ruta_base.is_dir(): continue
            
            # Buscar con el ID explícito (protege el root de imagenes_shorts de matching ciegos)
            audios = list(ruta_base.glob(f"*{fid}*MASTER.wav")) + list(ruta_base.glob(f"*{fid}*MASTER.mp3")) + \
                     list(ruta_base.glob(f"*{fid}*.wav")) + list(ruta_base.glob(f"*{fid}*.mp3"))
                     
            # Si estamos dentro de la carpeta local del ID, cualquier audio sirve
            if not audios and ruta_base.name == fid:
                audios = list(ruta_base.glob("*.wav")) + list(ruta_base.glob("*.mp3"))
        ...
        if not wav or not wav.exists():
            print(f"⚠️ [Assembler] Audio (wav/mp3) no encontrado para {fid}. Saltando.")
            continue
...
if __name__ == "__main__":
    ...
    # Buscar el audio (wav/mp3) en las carpetas
    wav = None
    posibles_rutas = [
        folder_clips, 
        folder_audio,
        Path(f"output/sesion_{args.id}"),
        Path("output/imagenes_shorts")
    ]
    for ruta_base in posibles_rutas:
        if not ruta_base.exists() or not ruta_base.is_dir(): continue
        
        # Buscar con el ID explícito
        audios = list(ruta_base.glob(f"*{args.id}*MASTER.wav")) + list(ruta_base.glob(f"*{args.id}*MASTER.mp3")) + \
                 list(ruta_base.glob(f"*{args.id}*.wav")) + list(ruta_base.glob(f"*{args.id}*.mp3"))
                 
        if not audios and ruta_base.name == args.id:
            audios = list(ruta_base.glob("*.wav")) + list(ruta_base.glob("*.mp3"))
    ...
    if not clips or not wav or not wav.exists():
        print(f"❌ Error: Faltan recursos en {folder_clips} o en output/sesion_{args.id}")
        if not clips: print("   - No se encontraron clips .mp4")
        if not wav or not wav.exists(): print(f"   - No se encontró ningún audio (.wav o .mp3) para el ID {args.id}")
    else:
